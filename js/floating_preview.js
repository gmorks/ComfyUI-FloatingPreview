// floating_preview.js
// ComfyUI Picture-in-Picture Widget with Configuration Support
// Place in: ComfyUI/custom_nodes/ComfyUI-FloatingPreview/js/floating_preview.js
...
async function loadConfig() {
    try {
        const response = await fetch('/extensions/ComfyUI-FloatingPreview/config.json');
        if (response.ok) {
            const userConfig = await response.json();
            console.log("✅ Floating Preview: Loaded user config");
            return { ...DEFAULT_CONFIG, ...userConfig };
        }
    } catch (e) {
        console.log("ℹ️ Floating Preview: No custom config found, using defaults");
    }
...
app.registerExtension({
    name: "FloatingPreview.PiP",
    
    async setup() {
        console.log("🖼️ Floating Preview: Initializing...");
        
        // Load configuration
        const config = await loadConfig();
        
        // Create the floating preview window
        const pip = new FloatingPreviewWindow(config);
        pip.create();
        
        // Listen for preview updates from ANY PreviewImage or SaveImage node
        api.addEventListener("executed", (event) => {
            ...
            if (output && output.images && output.images.length > 0) {
                ...
                console.log("🖼️ Floating Preview: Captured from node", node);
            }
        });
        ...
        window.addEventListener("keydown", (e) => {
            ...
            if (shortcut === config.shortcuts.toggle) {
                ...
                e.stopPropagation();
                pip.toggle();
                console.log("🎹 Floating Preview: Toggled via", config.shortcuts.toggle);
            }
        ...
        console.log("⌨️ Floating Preview: Keyboard shortcuts registered");
...
class FloatingPreviewWindow {
    ...
    getCurrentGridPosition() {
        ...
        return { row: 1, col: 0 }; // Default to middle-left
    }
    ...
    create() {
        ...
        this.container.id = "floating-preview-pip";
        ...
        placeholder.textContent = "Waiting for preview...";
    ...
    updateImage(imageInfo) {
        ...
        console.log("🖼️ Floating Preview: Updated image");
    }
...
console.log("✅ Floating Preview Extension Loaded");